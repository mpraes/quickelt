"""
Testes Automáticos para Templates de Ingestão usando Polars

Este módulo contém testes para validar se os templates de ingestão que utilizam Polars estão funcionando corretamente.

ORIENTAÇÕES:
- Cada teste importa o respectivo módulo de ingestão e executa a função ingest().
- O teste verifica se:
  - Um arquivo de dados (.parquet) foi gerado no diretório bronze.
...
- The test checks:
  - A data file (.parquet) was generated in the bronze directory.
